...
## ✅ EQEmu Editor Specific Notes

-   Domain config/constants: [config/everquest.php](config/everquest.php#L1-L40) — contains the canonical bitmasks, mappings, and links used across the app.
-   Helpers: `app/Helpers/EverquestHelper.php` is autoloaded via `composer.json` (`files`) and contains many convenience functions used in views and controllers. See [EverquestHelper.php](app/Helpers/EverquestHelper.php#L1-L60).
-   Many EverQuest mappings are centralized in `config/everquest.php` — change them there, not inline in controllers or views.
-   Helper functions (e.g., `eq_race`, `eq_class`, `get_class_usable_string`) are defined in `app/Helpers/EverquestHelper.php` and expected to be globally available.
