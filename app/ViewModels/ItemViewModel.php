...
class ItemViewModel
{
    ...
    public function withEffects()
    {
        ...
        $i->custom_bardeffect = $i->relationLoaded('bardEffectSpell') && $i->bardEffectSpell
            ? $i->bardEffectSpell->name
            : null;
    ...
    public function dropsByZone(): array
    {
        ...
        $results = DB::connection('eqemu')->table('items')
            ...
            ->leftJoin('spawnentry', function($se) {
                $se->on('npc_types.id', '=', 'spawnentry.npcID')
            ...
            ->when(!empty($ignoreZones), fn($q) => $q->where(function ($sub) use ($ignoreZones) {
        ...
        $results->transform(function ($row) use ($allZones) {
            if (empty($row->zone)) {
                $zoneGuess = (int) floor($row->npc_id / 1000);
                $zoneData = $allZones->firstWhere('zoneidnumber', $zoneGuess);
                if ($zoneData) {
                    $row->zone = $zoneData->short_name;
                    $row->version = $zoneData->version ?? 0;
                } else {
                    $row->zone = 'unknown';
                    $row->version = 0;
                }
        ...
        $grouped = $results->groupBy(fn($row) => strtolower($row->zone) . '-' . ($row->version ?? 0));
        ...
        foreach ($grouped as $zoneKey => $npcs) {
            ...
            $zoneData = $allZones
                ->where('short_name', $zoneShortName)
                ->where('version', (int) $version)
                ->first();

            if ($zoneShortName === 'unknown' || !$zoneData) {
                $zoneData = (object)[
                    'short_name' => 'unknown',
                    'long_name'  => 'Unknown Zone',
                    'version'    => 0,
                    'expansion'  => 0,
                ];
            }

            if (($zoneData->expansion ?? 0) > $currentExpansion) {
                continue;
            }
            ...
            $drops[] = [
                ...
                'npcs' => $npcs
                    ->unique('npc_name')
                    ->filter(fn($npc) => !empty(trim(NpcType::npcFixName($npc->npc_name ?? ''))))
                    ->map(function ($npc) {
        ...
        $drops_by_zone = collect($drops)->sortBy(function ($group) {
            return $group['zone'] === 'unknown' ? 'zzz' : $group['zone_name'];
        })->values();

        $allNpcs = collect($drops_by_zone)->flatMap(function ($zone) {
            return collect($zone['npcs'])->map(fn($npc) => array_merge($npc, [
                'zone'      => $zone['zone'],
        ...
        $sanitizedNpcs = $allNpcs->filter(fn($npc) =>
            isset($npc['id'], $npc['clean_name'], $npc['zone_name'], $npc['chance'])
        );
