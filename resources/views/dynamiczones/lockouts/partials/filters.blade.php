<form method="GET" action="{{ url()->current() }}" class="flex gap-2 items-end">
    <div class="w-60">
        ...
        <input type="text" name="dynamic_zone" value="{{ request('dynamic_zone') }}" class="input w-full"
            placeholder="Dynamic Zone ID or Name">
    </div>

    <div class="w-60">
        <label class="label label-text">Event</label>
        <input type="text" name="event_name" value="{{ request('event_name') }}" class="input w-full"
