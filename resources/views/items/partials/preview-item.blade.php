{{-- @include('items.partials.item', ['item' => $item, 'no_outer_div' => true]) --}}
...
@endphp
    ...
    <div class="flex justify-between items-start">
        ...
        <div class="bg-base-100 w-10 h-auto ml-4 item-icon item-{{ $item->icon }} shadow-[0_0_12px_var(--color-info)]"></div>
    </div>

    <div class="mt-2 space-y-1 text-sm text-gray-500">
        <div>{{ $item->tag_string }}</div>
        @if ($item->classes > 0)
    ...
    <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-gray-100">
        ...
        <div>
            <table class="w-full">
                ...
                <x-item-stat name="CORRUPT" :stat="$item->svcorruption" :stat2="$item->heroic_svcorrup" />
            </table>
        </div>
        <div>
            <table class="w-full">
    ...
    <div class="mt-6 text-sm">
        ...
        @if ($item->proceffect > 0 && $item->proceffect < 65535)
            <div>
                ...
                @if ($item->proclevel2 > 0)
                    <strong>Level:</strong> {{ $item->proclevel }}-{{ $item->proclevel2 }}
                @endif
                <br>
        ...
        @if ($item->worneffect > 0 && $item->worneffect < 65535)
            <div>
                ...
                @endif
                @if ($item->wornlevel > 0)
                    <strong>Level:</strong> {{ $item->wornlevel }}-{{ $item->wornlevel2 }}
                @endif
            </div>
        @endif
        {{-- focus effect --}}
        @if ($item->focuseffect > 0 && $item->focuseffect < 65535)
            <div>
                ...
                @if ($item->focuslevel > 0)
                    <strong>Level:</strong> {{ $item->focuslevel }}-{{ $item->focuslevel2 }}
                @endif
            </div>
        @endif
        {{-- bard effect (tooltip uses same Focus Effect: title) --}}
        @if ($item->bardeffect > 0 && $item->bardeffect < 65535)
            <div>
                <strong>Focus Effect:</strong>
                @if ($item->custom_bardeffect)
                    <x-spell-link
                        :spell_id="$item->bardeffect"
                        :spell_name="$item->custom_bardeffect"
                        :spell_icon="null"
                        spell_class="inline-flex"
                        :effects_only="1"
                    />
                ...
                @endif
                @if ($item->bardlevel > 0)
                    <strong>Level:</strong> {{ $item->bardlevel }}-{{ $item->bardlevel2 }}
                @endif
            </div>
        @endif
        {{-- clicky effect --}}
        @if ($item->clickeffect > 0 && $item->clickeffect < 65535)
            <div>
                ...
                @if ($item->clicklevel > 0)
                    <br><strong>Level:</strong> {{ $item->clicklevel }}-{{ $item->clicklevel2 }}
                @endif
